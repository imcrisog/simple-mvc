<div class="w-3/4 flex justify-center items-center">
    <div class="w-1/2">
        ...
        <form class="w-full justify-center space-y-4 items-center flex flex-col" action="<?= LOCALHOST ?>/inventory/update/<?= $section['id'] ?>/section" method="post">
            ...
            <select class="bg-gray-50 border border-gray-300 text-gray-900 text-center text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" name="user">
            <option selected value="<?= $users[0]['id'] ?>"><?= $users[0]['username'] ?></option>
                <?php foreach ($users as $key => $user) { ?>
                    <?php if ($user['id'] != $section['user_id'] && $user['id'] != $users[0]['id']) { ?>
                    <option class="py-2 px-2" value="<?= $user['id'] ?>"> <?= $user['username'] ?> </option>
                    <?php } ?>
                <?php } ?>
            </select>
